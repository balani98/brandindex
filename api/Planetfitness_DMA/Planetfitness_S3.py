...
def create_logger_object(log_level):
    ...
    try:
        ...
        file_name_without_path = datetime.datetime.now().strftime(
            "deployment_%Y-%m-%d_%H-%M.log"
        )
...
def lambda_handler(event, context):
    logger, file_name, file_name_without_path = create_logger_object(
        log_level=LOG_LEVEL
    )
    response = ""
