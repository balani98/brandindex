...
def configurations_variable():
    ...
    config_json['BRANDSINDEX_BRANDS'] = Config.get('BRANDS',
            'BRANDSINDEX_BRANDS')
    return config_json
...
output_status_message('start date and end date for this run are {} and {}'.format(start_date,
                      end_date))

configs = configurations_variable()

if execute_local:
    username = 'Anett.farkas@xmedia.com'
    password = 'GeaqyxAv2Ehna2!'
else:
...
path1 = '...\\BrandIndex_Empower\\'
with open(path1 + 'Empower.json', 'r') as f:
    content = f.read()
    content = content.replace('###start_date###',
...
for query in data['data']['queries']:
    ...
    index = index + 1
    sectors_and_regions.append({'region': query['entity']['region'],
                               'sector_id': query['entity']['sector_id'
                               ]})

sectors_and_regions = pd.DataFrame(sectors_and_regions)
sectors_and_regions = \
    sectors_and_regions.drop_duplicates(subset=['sector_id'],
        keep='first')
...
for queries in chunks(total_queries, max_queries_single_go):
    output_status_message('sleeping in seconds, {}'.format(wait_time_each_api_call_in_sec))
...
output_status_message('Successfully ran brand index analysis for the brand : {}'.format(brand))

df = enrich_data_frame(df, query_index, df_all_sectors,
                       df_sector_brands, has_dma)
...
if execute_local:
    outputname = \
        '{brand}_{start_date}_{end_date}.csv'.format(brand=brand,
