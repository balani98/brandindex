...
path1 = "C:\\Users\\deepanshu.balani\\OneDrive - Nabler Web Solutions Pvt. Ltd\\Documents\\BrandIndex Crossmedia\\BrandIndex_USBHealthAPI\\"
with open(path1 + "USBHealth.json", "r") as f:
    content = f.read()
...
if brand_aggregation == "true":
    df_renamed = df.replace(to_replace=['Wells Fargo','Chase','Citibank','Bank of America',
                                        'Capital One Bank','PNC Bank','US Bank'], 
                            value='Large Competitive Set', inplace=False)
    df_renamed = df_renamed.replace(to_replace=['Fifth-Third','TCF Bank','Truist','Regions Bank'], 
                            value='Small Competitive Set', inplace=False) 
