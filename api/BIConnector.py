...
BI_BRANDS_IN_SECTORS_URL = 'https://api.brandindex.com/v1/taxonomies/regions/{}/sectors/{}/brands'
...
def authenticate(username,password):
    ...
    response = session.post(AUTH_BRANDINDEX_API_URL,
...
def run_analysis(session, payload):    
    ...
    response = session.post(BI_ANALYSIS_CSV_EXPORT_URL,
                            data=json.dumps(payload), headers=headers)
    return response

def get_sectors(session, region):
    response = session.get(
       BI_SECTORS_URL.format(region), headers=headers)
    return response
